...
def review_failure_engine(failure: ReviewFailure, fallback: str) -> str:
    return failure.engine if failure.engine else fallback
...
def load_ai_review_settings() -> dict[str, Any]:
    if not AI_REVIEW_SETTINGS_FILE.is_file():
        return copy.deepcopy(DEFAULT_AI_REVIEW_SETTINGS)
    try:
        with AI_REVIEW_SETTINGS_FILE.open(encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, json.JSONDecodeError) as error:
        raise ReviewFailure(f"Cannot read {AI_REVIEW_SETTINGS_FILE}: {error}") from error
...
def gh_json(args: list[str], engine: str) -> Any:
    ...
    try:
        ...
        raise ReviewFailure(f"gh returned invalid JSON: {error}", engine) from error
...
def load_qoder_settings() -> dict[str, Any]:
    ...
    try:
        ...
        raise ReviewFailure(f"Cannot read {path}: {error}", "qoder") from error
    if not isinstance(data, dict):
...
def configured_qoder_models(settings: dict[str, Any]) -> list[str]:
    ...
    implicit_auto_fallback = qoder_settings.get("implicit_auto_fallback")
    if not isinstance(implicit_auto_fallback, bool) or not implicit_auto_fallback:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        engine = review_failure_engine(exc, engine)
