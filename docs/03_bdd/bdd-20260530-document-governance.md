...
Feature: Dogsquard document governance
  ...
  Scenario: Machine-consumed agent contracts keep stable paths
    Given an installed project skill requires a document under `docs/agents/`
    When documentation is reorganized
    Then the agent contract may remain under `docs/agents/`
    And its filename must use lowercase kebab-case
    And it must include the required governance metadata
