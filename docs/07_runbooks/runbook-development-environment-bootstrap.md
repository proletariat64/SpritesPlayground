...
The command checks every component first. Healthy add-ons and Pi packages are skipped without downloading. Missing or damaged add-ons are restored from pinned release artifacts, missing Pi packages are installed from pinned sources with `pi install`, and then Godot runs a headless editor import.
...
This deliberately reinstalls both add-ons. To avoid downloads, combine `--repair` with both archive arguments.
...
Isolate Godot/add-on checks from external Pi package checks:
...
- `addons/limboai/`, `addons/godot_ai/`, `.pi/npm/`, and `.pi/git/` are local installations and must remain ignored and untracked.
