...
func _run() -> void:
	...
	var runtime_active_start := int(panel.moves_json["basic_punch"]["active_window"].get("start_frame", 0))
	panel.runtime_start_selected_move()
	panel.runtime_advance_frame(runtime_active_start)
	...
	errors.append_array(_expect(int(summary["current_frame"]) == runtime_active_start, "runtime advances selected move"))
...
func _run_npc_ui_smoke(panel: PanelContainer) -> Array:
	...
	panel._on_add_npc_pressed()
	errors.append_array(_expect(add_requests.size() == 2, "Add NPC emits even when cached count is max"))
	errors.append_array(_expect(str(panel.npc_status).contains("requested"), "Add NPC defers max guard to playground"))
	...
	panel._on_remove_selected_npc_pressed()
	errors.append_array(_expect(remove_requests.size() == 2, "Remove selected NPC emits even when cached count is min"))
	errors.append_array(_expect(str(panel.npc_status).contains("requested"), "Remove NPC defers min guard to playground"))
...
func _run_preview_smoke(panel: PanelContainer) -> Array:
	...
	var mismatch_row: Dictionary = panel._coverage_row_for("basic_punch")
	var mismatch_sequence_ref := str(mismatch_row.get("frame_sequence_ref", ""))
	var mismatch_original_sequence: Array = panel.sprite_set_json["frame_sequences"][mismatch_sequence_ref].duplicate(true)
	var mismatch_original_frame_count := int(panel.moves_json["basic_punch"].get("frame_count", 1))
	var mismatch_move_frame_count: int = mismatch_original_sequence.size() + 2
	panel.sprite_set_json["frame_sequences"][mismatch_sequence_ref] = mismatch_original_sequence.slice(0, 2)
	panel.set_move_scalar("frame_count", mismatch_move_frame_count)
	panel.preview_last()
	errors.append_array(_expect(panel.preview_frame_count() == mismatch_move_frame_count, "preview frame count includes move-only trailing frames"))
	errors.append_array(_expect(panel.preview_frame == mismatch_move_frame_count - 1, "preview last reaches move-only trailing frame"))
	errors.append_array(_expect(int(panel.action_preview_control.frame_count()) == mismatch_move_frame_count, "preview control frame count includes move-only trailing frames"))
	errors.append_array(_expect(str(panel.action_preview_control.current_render_state()) == "MISSING", "preview shows move-only trailing frame as missing"))
	...
	var original_frame_count := int(panel.moves_json["basic_punch"].get("frame_count", 1))
	errors.append_array(_expect(not panel.insert_empty_frame_slot(sequence_ref, 0, false), "frame slot insert requires shift choice"))
	errors.append_array(_expect(str(panel.status_label.text).contains("choose shift timing"), "frame slot insert reports shift requirement"))
	errors.append_array(_expect(panel.insert_empty_frame_slot(sequence_ref, 0, true), "frame slot insert with shift succeeds"))
	errors.append_array(_expect(panel.sprite_set_json["frame_sequences"][sequence_ref].size() == original_sequence.size() + 1, "frame slot insert grows sequence"))
	errors.append_array(_expect(int(panel.moves_json["basic_punch"].get("frame_count", 1)) == original_frame_count + 1, "frame slot insert shifts frame count"))
	panel.sprite_set_json["frame_sequences"][sequence_ref] = original_sequence.duplicate(true)
	panel.set_move_scalar("frame_count", original_frame_count)
	errors.append_array(_expect(not panel.remove_frame_slot(sequence_ref, 3), "frame slot delete blocks referenced frame"))
	errors.append_array(_expect(str(panel.status_label.text).contains("references frame"), "frame slot delete reports reference block"))
	errors.append_array(_expect(panel.mark_frame_slot(sequence_ref, 0, "empty"), "frame slot marks empty"))
	panel.preview_reset()
	panel._refresh_fields()
	errors.append_array(_expect(str(panel.action_preview_control.current_render_state()) == "EMPTY", "preview renders empty slot"))
	errors.append_array(_expect(panel.mark_frame_slot(sequence_ref, 0, "missing"), "frame slot marks missing"))
	panel.preview_reset()
	panel._refresh_fields()
	errors.append_array(_expect(str(panel.action_preview_control.current_render_state()) == "MISSING", "preview renders explicit missing slot"))
	errors.append_array(_expect(panel.mark_frame_slot(sequence_ref, 0, "placeholder"), "frame slot marks placeholder"))
	panel.preview_reset()
	panel._refresh_fields()
	errors.append_array(_expect(str(panel.action_preview_control.current_render_state()) == "PLACEHOLDER", "preview renders marked placeholder slot"))
	panel.replace_frame_slot(sequence_ref, 0, "res://missing/replace_stale_source.png")
	panel.set_preview_frame(1)
	errors.append_array(_expect(panel.frame_slot_path_input == null or str(panel.frame_slot_path_input.text) == str(panel.sprite_set_json["frame_sequences"][sequence_ref][1]), "frame slot input refreshes after scrub"))
	panel.sprite_set_json["frame_sequences"][sequence_ref] = original_sequence.duplicate(true)
	errors.append_array(_expect(not panel.has_method("move_frame_slot"), "frame slot reorder is not exposed"))
