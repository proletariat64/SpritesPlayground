...
func _run() -> void:
	...
	var reloaded_template: Dictionary = DataStore.load_template(copy_id)
	var reloaded_move: Dictionary = DataStore.load_move("basic_punch")
	errors.append_array(_expect(not reloaded_template.is_empty(), "reloaded template exists"))
	errors.append_array(_expect(not reloaded_move.is_empty(), "reloaded move exists"))
	if not reloaded_template.is_empty():
		var reloaded_hurtboxes: Dictionary = reloaded_template.get("hurtboxes", {})
		var reloaded_hurt_head: Dictionary = reloaded_hurtboxes.get("hurt_head", {})
		errors.append_array(_expect(int(reloaded_template.get("hp", -1)) == 111, "reloaded hp"))
		errors.append_array(_expect(float(reloaded_hurt_head.get("x", INF)) == -10.0, "reloaded hurtbox"))
	if not reloaded_move.is_empty():
		errors.append_array(_expect(int(reloaded_move.get("damage", -1)) == 9, "reloaded move damage"))
		errors.append_array(_expect(bool(reloaded_move.get("multi_hit", false)), "reloaded multi_hit"))
		errors.append_array(_expect(str(reloaded_move.get("move_type", "")) == "combat", "reloaded move_type"))
		errors.append_array(_expect(str(reloaded_move.get("state_context_override", "")) == "idle", "reloaded state_context_override"))
...
func _run_preview_smoke(panel: PanelContainer) -> Array:
	...
	var original_frame_path := str(panel.sprite_set_json["frame_sequences"][sequence_ref][0])
	...
	errors.append_array(_expect(panel.replace_frame_slot(sequence_ref, 0, "res://missing/replace_stale_source.png"), "frame slot replacement accepts a supported path"))
	...
	errors.append_array(_expect(panel.replace_frame_slot(sequence_ref, 0, original_frame_path), "frame slot replacement restores the authored fixture"))
	...
	errors.append_array(_expect(panel.replace_frame_slot(sequence_ref, 0, texture_path), "frame slot replacement accepts a local Preview texture"))
	panel.preview_reset()
	...
	errors.append_array(_expect(panel.replace_frame_slot(sequence_ref, 0, "res://missing/creator_lab_preview_smoke.png"), "frame slot replacement accepts an explicit missing texture path"))
	panel.preview_reset()
	errors.append_array(_expect(str(panel.action_preview_control.current_render_state()) == "MISSING", "preview renders missing frame"))
	errors.append_array(_expect(panel.replace_frame_slot(sequence_ref, 0, original_frame_path), "frame slot replacement restores the original Preview frame"))
